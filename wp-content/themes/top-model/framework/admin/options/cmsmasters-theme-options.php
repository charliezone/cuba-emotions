 ...
 * @version 	1.0.8
 * 
 * Post, Page, Project, Profile, Product, Timetable Events, Donation & Donations Campaign Options
...
if (CMSMASTERS_TIMETABLE && function_exists('timetable_events_init')) {
	require_once(CMSMASTERS_OPTIONS . '/cmsmasters-theme-options-tt-event.php');
}


if (CMSMASTERS_EVENTS_CALENDAR && class_exists('Tribe__Events__Main')) {
	require_once(CMSMASTERS_OPTIONS . '/cmsmasters-theme-options-tribe-event.php');
}
...
if (class_exists('Cmsmasters_Sermons')) {
	require_once('cmsmasters-sermons/admin/cmsmasters-theme-options-sermon.php');
}

require_once(CMSMASTERS_OPTIONS . '/cmsmasters-theme-options-product.php');


if (!function_exists('top_model_get_custom_all_meta_fields')) {
function top_model_get_custom_all_meta_fields() {
	$custom_meta_fields = top_model_get_custom_general_meta_fields();
	...
	if ( 
		...
		$custom_page_meta_fields = top_model_get_custom_page_meta_fields();
		...
		$custom_project_meta_fields = top_model_get_custom_project_meta_fields();
		...
		$custom_profile_meta_fields = top_model_get_custom_profile_meta_fields();
		...
		$custom_tt_event_meta_fields = top_model_get_custom_tt_event_meta_fields();
		...
		$custom_product_meta_fields = top_model_get_custom_product_meta_fields();
		...
		((
			(isset($_GET['post_type']) && $_GET['post_type'] == 'tribe_events') || 
			(isset($_POST['post_type']) && $_POST['post_type'] == 'tribe_events') || 
			(isset($_GET['post']) && get_post_type($_GET['post']) == 'tribe_events') 
		) || (
			(isset($_GET['post_type']) && $_GET['post_type'] == 'tribe_venue') || 
			(isset($_POST['post_type']) && $_POST['post_type'] == 'tribe_venue') || 
			(isset($_GET['post']) && get_post_type($_GET['post']) == 'tribe_venue') 
		) || (
			(isset($_GET['post_type']) && $_GET['post_type'] == 'tribe_organizer') || 
			(isset($_POST['post_type']) && $_POST['post_type'] == 'tribe_organizer') || 
			(isset($_GET['post']) && get_post_type($_GET['post']) == 'tribe_organizer') 
		))
	) {
		$custom_tribe_event_meta_fields = top_model_get_custom_tribe_event_meta_fields();
		
		$custom_all_meta_fields = array_merge($custom_tribe_event_meta_fields, $custom_meta_fields);
	} elseif ( 
		...
		$custom_post_meta_fields = top_model_get_custom_post_meta_fields();
		
		$custom_all_meta_fields = array_merge($custom_post_meta_fields, $custom_meta_fields);
	} else {
		$custom_other_meta_fields = top_model_get_custom_other_meta_fields();
...
function top_model_admin_enqueue_scripts($hook) {
	if ( 
		($hook == 'post.php') || 
		($hook == 'post-new.php') 
	) {
		...
		wp_enqueue_style('top-model-options-css', get_template_directory_uri() . '/framework/admin/options/css/cmsmasters-theme-options.css', array(), '1.0.0', 'screen');
		
		
		if (is_rtl()) {
			wp_enqueue_style('top-model-options-css-rtl', get_template_directory_uri() . '/framework/admin/options/css/cmsmasters-theme-options-rtl.css', array(), '1.0.0', 'screen');
		}
		
		
		wp_enqueue_script('top-model-options-js', get_template_directory_uri() . '/framework/admin/options/js/cmsmasters-theme-options.js', array('jquery'), '1.0.0', true);
		
		wp_localize_script('top-model-options-js', 'cmsmasters_options', array( 
			'palettes' => 			implode(',', cmsmasters_color_picker_palettes()), 
			'create_gallery' => 	esc_attr__('Create Gallery', 'top-model'), 
			'select_format' => 		esc_attr__('Please select the format.', 'top-model'), 
			'link_exists' => 		esc_html__('Link with this format already exists.', 'top-model'), 
			'want_remove' => 		esc_html__('Do you really want to remove this item?', 'top-model'), 
		...
		wp_enqueue_script('top-model-options-js-toggle', get_template_directory_uri() . '/framework/admin/options/js/cmsmasters-theme-options-toggle.js', array('jquery'), '1.0.0', true);
	}
}

add_action('admin_enqueue_scripts', 'top_model_admin_enqueue_scripts');


function top_model_show_meta_box() {
	global $post;
	
	$custom_all_meta_fields = top_model_get_custom_all_meta_fields();
	...
	foreach ($custom_all_meta_fields as $field) {
		...
		case 'select_sidebar':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Default Sidebar', 'top-model') . '</option>';
		...
		case 'select_slider':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Slider', 'top-model') . '</option>';
		...
		case 'select_post_categ':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Blog Category', 'top-model') . '</option>';
		...
		case 'select_project_cat':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Project Category', 'top-model') . '</option>';
		...
		case 'select_pl_categ':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Profile Category', 'top-model') . '</option>';
		...
		case 'image':
			...
			echo '<div class="cmsmasters_upload_parent cmsmasters_select_parent">' . 
				'<input type="button" id="cmsmasters_upload_' . $field['id'] . '_button" class="cmsmasters_upload_button button button-large" value="' . esc_attr__('Choose Image', 'top-model') . '" data-title="' . esc_attr__('Choose Image', 'top-model') . '" data-button="' . esc_attr__('Insert Image', 'top-model') . '" data-id="cmsmasters-media-select-frame-' . $field['id'] . '" data-classes="media-frame cmsmasters-media-select-frame' . ((!isset($field['description'])) ? ' cmsmasters-frame-no-description' : '') . ((!isset($field['caption'])) ? ' cmsmasters-frame-no-caption' : '') . ((!isset($field['align'])) ? ' cmsmasters-frame-no-align' : '') . ((!isset($field['link'])) ? ' cmsmasters-frame-no-link' : '') . ((!isset($field['size'])) ? ' cmsmasters-frame-no-size' : '') . '" data-library="image" data-type="' . $field['frame'] . '"' . (($field['frame'] === 'post') ? ' data-state="insert"' : '') . ' data-multiple="' . $field['multiple'] . '" />' . 
		...
		case 'icon':
			echo '<div class="icon_management">' . 
				'<p>' . 
					...
					'<input id="' . $field['id'] . '_button" class="cmsmasters_icon_choose_button button" type="button" value="' . esc_attr__('Choose icon', 'top-model') . '" />' . 
					'<a href="#" class="cmsmasters_remove_icon admin-icon-remove" title="' . esc_attr__('Remove icon', 'top-model') . '"' . (($meta != '') ? ' style="display:inline-block;"' : '') . '></a>' . 
				'</p>' . 
			'</div>' . 
		...
		case 'repeatable_link':
			...
			echo '<div class="ovh">' . 
				'<div class="fl"><strong>' . esc_html__('Title', 'top-model') . '</strong></div>' . 
				'<div class="fl"><strong>' . esc_html__('Link', 'top-model') . '</strong></div>' . 
			'</div>' . 
			...
			'<select name="' . $field['id'] . '-select" id="' . $field['id'] . '-select">' . 
				'<optgroup label="' . esc_attr__('Blank Field', 'top-model') . '">' . 
					'<option value="">' . esc_html__('Select Link', 'top-model') . '</option>' . 
				'</optgroup>' . 
				'<optgroup label="' . esc_attr__('Posts', 'top-model') . '">';
			...
			echo '</optgroup>' . 
				'<optgroup label="' . esc_attr__('Pages', 'top-model') . '">';
			...
			echo '</optgroup>' . 
				'<optgroup label="' . esc_attr__('Projects', 'top-model') . '">';
		...
		case 'repeatable_multiple':
			echo '<div class="ovh">' . 
				'<div class="fl"><strong>' . esc_html__('Title', 'top-model') . '</strong></div>' . 
				'<div class="fl"><strong>' . esc_html__('Values', 'top-model') . '</strong></div>' . 
			'</div>' . 
			'<ul id="' . $field['id'] . '-repeatable" class="custom_repeatable">';
		...
		case 'repeatable_media':
			echo '<select name="' . $field['id'] . '-select" id="' . $field['id'] . '-select">' . 
				'<option value="">' . esc_html__('Select Format', 'top-model') . ' &nbsp;</option>';
		...
		case 'images_list':
			...
			echo '<div class="cmsmasters_upload_parent cmsmasters_gallery_parent">' . 
				'<input type="button" id="cmsmasters_gallery_' . $field['id'] . '_button" class="cmsmasters_upload_button button button-large" value="' . (($meta != '') ? esc_attr__('Edit Gallery', 'top-model') : esc_attr__('Create Gallery', 'top-model')) . '" data-title="' . esc_attr__('Create/Edit Gallery', 'top-model') . '" data-button="' . esc_attr__('Insert Gallery', 'top-model') . '" data-id="cmsmasters-media-select-frame-' . $field['id'] . '" data-classes="media-frame cmsmasters-media-gallery-frame' . ((!isset($field['description'])) ? ' cmsmasters-frame-no-description' : '') . ((!isset($field['caption'])) ? ' cmsmasters-frame-no-caption' : '') . ((!isset($field['align'])) ? ' cmsmasters-frame-no-align' : '') . ((!isset($field['link'])) ? ' cmsmasters-frame-no-link' : '') . ((!isset($field['size'])) ? ' cmsmasters-frame-no-size' : '') . '" data-library="image" data-type="' . $field['frame'] . '"' . (($field['frame'] == 'post') ? ' data-state="' . (($meta != '') ? 'gallery-edit' : 'gallery-library') . '"' : '') . ' data-multiple="' . $field['multiple'] . '"' . (($meta != '') ? ' data-editing="true"' : '') . ' />' . 
				...
				foreach ($ids as $id) {
					if (isset($id[0]) && isset($id[1])) {
						echo '<li class="cmsmasters_gallery_item">' . 
							...
							'<a href="#" class="cmsmasters_gallery_cancel admin-icon-remove" title="' . esc_attr__('Remove', 'top-model') . '"></a>' . 
						'</li>';
					}
				}
			}
		...
		case 'funds':
			...
			echo '<div class="cmsmasters_funds_stats">' . 
				'<div class="cmsmasters_funds_stat_wrap">' . 
					...
					'</div>' . 
				'</div>' . 
				'<span class="fl">' . esc_html__('count', 'top-model') . ' - <strong>' . $funds_number . '</strong></span>' . 
				'<span class="fr">' . cmsmasters_donations_currency(cmsmasters_number_format($target - $funds)) . ' ' . esc_html__('to go', 'top-model') . '</span>';
			'</div>' . 
			(($field['desc'] != '') ? '<br />' . '<span class="description">' . $field['desc'] . '</span>': '');
...
function top_model_save_custom_meta($post_id) {
    $custom_all_meta_fields = top_model_get_custom_all_meta_fields();
...
add_action('save_post', 'top_model_save_custom_meta');


function top_model_add_custom_cmsmasters_meta_box() {
	$args = array(
	   'public' => true 
	);
	...
	foreach ($screens as $screen) {	
		add_meta_box( 
			...
			esc_html__('Theme Options', 'top-model'), 
			'top_model_show_meta_box', 
			$screen, 
			'normal', 
			'high' 
		);
...
add_action('add_meta_boxes', 'top_model_add_custom_cmsmasters_meta_box');
