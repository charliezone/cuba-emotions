...
function top_model_woocommerce_cart_dropdown() {
	...
	if (
		!isset($cmsmasters_option['top-model' . '_woocommerce_cart_dropdown']) ||
		(
			isset($cmsmasters_option['top-model' . '_woocommerce_cart_dropdown']) &&
			!$cmsmasters_option['top-model' . '_woocommerce_cart_dropdown']
		)
	) {
		echo '<div class="cmsmasters_dynamic_cart">' .  
			'<a href="' . esc_url(wc_get_cart_url()) . '" class="cmsmasters_dynamic_cart_button"><span class="cmsmasters_theme_icon_basket">' . esc_html($cart_count) . '</span></a>' . 
...
function top_model_woocommerce_add_to_cart_button() {
	...
	if ( 
		...
		echo '<div class="button_to_cart_wrap">' . 
			'<a href="' . esc_url($product->add_to_cart_url()) . '" data-product_id="' . esc_attr($product->get_id()) . '" data-product_sku="' . esc_attr($product->get_sku()) . '" class="button_to_cart add_to_cart_button cmsmasters_add_to_cart_button ajax_add_to_cart product_type_simple" title="' . esc_attr__('Add to Cart', 'top-model') . '">' . 
				'<span>' . esc_html__('Add to Cart', 'top-model') . '</span>' . 
			'</a>' . 
			'<a href="' . esc_url(wc_get_cart_url()) . '" class="button_to_cart added_to_cart wc-forward" title="' . esc_attr__('View Cart', 'top-model') . '">' . 
...
function top_model_woocommerce_demo_store($html, $notice) {
...
add_filter('woocommerce_demo_store', 'top_model_woocommerce_demo_store', 10, 2);


function top_model_woocommerce_support() {
...
add_action('after_setup_theme', 'top_model_woocommerce_support');
...
function top_model_change_on_sale() {
	return '<span class="onsale"><span>' . esc_html__('Sale!', 'top-model') . '</span></span>';
}


/*  Dynamic cart count update after ajax */
function woocommerce_header_add_to_cart_count( $dynamic_count ) {
	...
	?>
	<span class="cmsmasters_theme_icon_basket"><?php echo top_model_return_content($woocommerce->cart->cart_contents_count); ?></span>
	<?php
