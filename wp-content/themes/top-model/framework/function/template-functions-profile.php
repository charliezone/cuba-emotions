...
function top_model_profile_heading($cmsmasters_id, $tag = 'h1', $sub_title = false, $tag_sub = 'h3', $show = true, $profile_mode = 'default') { 
...
function top_model_profile_title_nolink($cmsmasters_id, $tag = 'h1', $sub_title = false, $tag_sub = 'h5', $show = true) { 
	$out = '<' . esc_html($tag) . ' class="cmsmasters_profile_title entry-title">' . 
...
function top_model_profile_exc_cont($show = true) {
...
function top_model_get_profile_category($cmsmasters_id, $taxonomy, $template_type = 'page', $show = true) {
	$out = '';
	...
	if (get_the_terms($cmsmasters_id, $taxonomy)) {
		if ($template_type == 'page') {
			...
			if ($cmsmasters_option['top-model' . '_profile_post_cat']) {
				$out .= '<div class="profile_details_item">' . 
					'<div class="profile_details_item_title">' . esc_html__('Categories', 'top-model') . ':' . '</div>' . 
...
function top_model_get_profile_likes($template_type = 'page', $show = true) {
	$out = '';
	...
	if ($template_type == 'page') {
		...
		if ($cmsmasters_option['top-model' . '_profile_post_like']) {
			$out = '<div class="profile_details_item">' . 
				'<div class="profile_details_item_title">' . esc_html__('Likes', 'top-model') . ':' . '</div>' . 
...
function top_model_get_profile_comments($template_type = 'page', $show = true) {
	$out = '';
	
	
	if (comments_open()) {
		if ($template_type == 'page') {
			$out = top_model_get_comments('cmsmasters_profile_comments');
		} elseif ($template_type == 'post') {
			$cmsmasters_option = top_model_get_global_options();
			
			if ($cmsmasters_option['top-model' . '_profile_post_comment']) {
				$out .= '<div class="profile_details_item">' . 
					'<div class="profile_details_item_title">' . esc_html__('Comments', 'top-model') . ':' . '</div>' . 
					'<div class="profile_details_item_desc details_item_desc_comments">' . 
						top_model_get_comments('cmsmasters_profile_comments') . 
					'</div>' . 
				'</div>';
			}
		}
...
function top_model_get_profile_features($features_position = 'features', $features = '', $features_title = false, $tag = 'h2', $show = true) {
	if (
		(
			!empty($features[0][0]) && 
			!empty($features[0][1])
...
function top_model_profile_social_icons($social_icons, $shortcode_id = false, $title_box = false, $tag = 'h2', $show = true) {
	if ($social_icons != '') {
		$out = '<div class="profile_social_icons">';
			...
			if ($social_icon_styles != '') {
				echo cmsmasters_profiles_generate_front_css($social_icon_styles);
			}
...
function top_model_profile_open_social_icons($social_icons, $profile_id = false, $title_box = false, $tag = 'h2', $show = true) {
	if ($social_icons != '') {
		$out = '<div class="profile_social_icons">';
...
function top_model_profile_open_style_social_icons() {
	...
	if ($social_icons != '') {
		...
		if ($social_icon_styles != '') {
			wp_add_inline_style('top-model-retina', $social_icon_styles);
		}
	}
}

add_action('wp_enqueue_scripts', 'top_model_profile_open_style_social_icons');
